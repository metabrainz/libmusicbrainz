 ...
 * (see lgpl.txt) along with this library; if not, write to the Free Software
...
#ifndef WIN32
  #ifdef _UNICODE
    int _tcslen(LPCTSTR c)   { return wcslen(c); }
    int _tcsnicmp(LPCTSTR c1, LPCTSTR c2, int l) { return wcsncasecmp(c1,c2,l);}
    int _tcsicmp(LPCTSTR c1, LPCTSTR c2) { return wcscasecmp(c1,c2); }
    LPTSTR _tcsstr(LPCTSTR c1, LPCTSTR c2) { return (LPTSTR)wcsstr(c1,c2); }
    LPTSTR _tcschr(LPCTSTR c1, int c2) { return (LPTSTR)wcschr(c1,c2); }
    LPTSTR _tcscpy(LPTSTR c1, LPCTSTR c2) { return (LPTSTR)wcscpy(c1,c2); }
  #else
    int _tcslen(LPCTSTR c)   { return strlen(c); }
    int _tcsnicmp(LPCTSTR c1, LPCTSTR c2, int l) { return strncasecmp(c1,c2,l);}
    int _tcsicmp(LPCTSTR c1, LPCTSTR c2) { return strcasecmp(c1,c2); }
    LPTSTR _tcsstr(LPCTSTR c1, LPCTSTR c2) { return (LPTSTR)strstr(c1,c2); }
    LPTSTR _tcschr(LPCTSTR c1, int c2) { return (LPTSTR)strchr(c1,c2); }
    LPTSTR _tcscpy(LPTSTR c1, LPCTSTR c2) { return (LPTSTR)strcpy(c1,c2); }
  #endif
#endif
...
void XMLNode::exactMemory(XMLNodeData *d)
{
    ...
    if (d->pText)      d->pText=(LPCTSTR*)realloc(d->pText,d->nText*sizeof(LPTSTR));
...
#ifdef __linux__
#ifdef _UNICODE
    // The 5 next lines convert multibyte character strings to wide characters.
    // These lines are only useful if the file is not already in UNICODE.
    const char *p=buf;
    LPTSTR b2=(LPTSTR)malloc(l*sizeof(TCHAR)+sizeof(TCHAR));
    mbsrtowcs(b2,&p,l,0); // this is the GCC equivalent of MultiByteToWideChar(). REVISIT: Check for return value
    free(buf);
    buf=(char*)b2;
#endif
#elif defined(WIN32)
...
{
    if (!d) return;
    int i,l=d->nAttribute;
    XMLAttribute *pa=d->pAttribute;
    for (i=0; i<l; i++) if (lpszName==pa[i].lpszName) { deleteAttribute(i); return; }
}

void XMLNode::deleteAttribute(XMLAttribute *a){ if (a) deleteAttribute(a->lpszName); }
void XMLNode::deleteAttributeRelaxed(LPCTSTR lpszName)
{
    int j=0;
...
void XMLNode::deleteClear(LPCTSTR lpszValue)
{
    if (!d) return;
    int i,l=d->nClear;
    XMLClear *p=d->pClear;
